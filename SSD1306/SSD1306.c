...
#include "common.h"
	...
	// SPI handle
	extern nrf_drv_spi_t m_spi_master;
#else
...
#define getRotation	Adafruit_GFX_getRotation
...
// mahesh - not supporting reset yet
  ...
  if (reset) {
    ...
    nrf_delay_ms(1);
    // bring reset low
    nrf_gpio_pin_clear(SSD1306_SPI_RES_PIN);
    // wait 10ms
    nrf_delay_ms(10);
    // bring out of reset
    nrf_gpio_pin_set(SSD1306_SPI_RES_PIN);
...
void SSD1306_command(uint8_t c)
{
	...
	nrf_gpio_pin_clear(SSD1306_SPI_DC_PIN);
	nrf_gpio_pin_clear(SSD1306_SPI_CS_PIN);
	
	ret = nrf_drv_spi_transfer(&m_spi_master, &c, 1, NULL, 0);
	APP_ERROR_CHECK(ret);
...
void SSD1306_display(void) {
  ...
  SSD1306_command(0); // Page start address (0 = reset)
	...
	for (uint16_t i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i+=128) {
		ret = nrf_drv_spi_transfer(&m_spi_master, &buffer[i], 128, NULL, 0);
		APP_ERROR_CHECK(ret);
    }
